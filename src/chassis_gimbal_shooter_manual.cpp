  ...
  : ChassisGimbalManual(nh, nh_referee)
{
  ...
  ballistic_solution_sub_ =
      nh.subscribe<std_msgs::Float32MultiArray>("/controllers/gimbal_controller/ballistic_solution", 10,
                                                &ChassisGimbalShooterManual::ballisticSolutionCallback, this);
...
void ChassisGimbalShooterManual::mouseRightPress()
{
  ...
  if (track_data_.id == 6 && track_data_.tracking == 1)
  {
    if (shooter_cmd_sender_->getMsg()->mode != rm_msgs::ShootCmd::STOP)
    {
      shooter_cmd_sender_->setMode(rm_msgs::ShootCmd::PUSH);
...
void ChassisGimbalShooterManual::xPress()
{
  ...
  double roll_mapping{}, pitch_mapping{}, yaw_mapping{};
  try
  {
    quatToRPY(tf_buffer_.lookupTransform("base_link", "yaw", ros::Time(0)).transform.rotation, roll_current,
              pitch_current, yaw_current);
    quatToRPY(tf_buffer_.lookupTransform("map", "base_link", ros::Time(0)).transform.rotation, roll_mapping,
              pitch_mapping, yaw_mapping);
  }
  ...
  gimbal_cmd_sender_->setGimbalTraj(yaw_current - yaw_mapping, pitch_current);
}
